...
namespace :cpi do
  ...
  task refresh: :environment do
    ...
    raw_series.each do |entry|
      next unless entry["period"] == "M13" # M13 is the annual average row
      annual_averages[entry["year"].to_s] = entry["value"].to_f
    end
    ...
    write_data_file(series_id, merged)
...
def write_data_file(series_id, annuals)
  path = Rails.root.join("db", "cpi_data.json")
  payload = {
    "series_id"        => series_id,
